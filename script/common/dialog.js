...
export async function prepareCommonRoll(rollData) {
    const html = await renderTemplate("systems/only-war/template/dialog/common-roll.hbs", rollData);
...
export async function prepareCombatRoll(rollData, actorRef) {
    if (rollData.weapon.isRanged && rollData.weapon.clip.value <= 0) {
        ...
        const html = await renderTemplate("systems/only-war/template/dialog/combat-roll.hbs", rollData);
        let dialog = new Dialog({
            title: rollData.name,
            content: html,
            buttons: {
...
export async function preparePsychicPowerRoll(rollData) {
    const html = await renderTemplate("systems/only-war/template/dialog/psychic-power-roll.hbs", rollData);
    let dialog = new Dialog({
        title: rollData.name,
        content: html,
        buttons: {
